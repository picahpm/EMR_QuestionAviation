...
{
    public partial class AviationAirCrewFAA : UserControl
    {
        public AviationAirCrewFAA()
        {
            InitializeComponent();
        }
            ...
            set
            {
                ...
                else
                {
                    try
                    {
                        ...
                        trn_eye_aircrew_faa eye_aircrew_faa = patient_eyes.trn_eye_aircrew_faas.FirstOrDefault();
                        if (eye_aircrew_faa == null)
                        {
                            eye_aircrew_faa = new trn_eye_aircrew_faa();
                            patient_eyes.trn_eye_aircrew_faas.Add(eye_aircrew_faa);
                        }

                        load_aircrew_faa(ref eye_aircrew_faa);
        ...
        private void patient_eyes_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == "teh_vision_out_re" || e.PropertyName == "teh_vision_out_le" || e.PropertyName == "teh_color_vision" || e.PropertyName == "teh_vision_re" || e.PropertyName == "teh_vision_le")
            {
                ...
                {
                    bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_dvis_right = (string)val;
                }
                if (e.PropertyName == "teh_vision_out_le")
                {
                    bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_dvis_left = (string)val;
                }
                if (e.PropertyName == "teh_vision_re")
                {
                    bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_dvis_rcorrect = (string)val;
                }
                if (e.PropertyName == "teh_vision_le")
                {
                    bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_dvis_lcorrect = (string)val;
                }
                if (e.PropertyName == "teh_color_vision")
                {
                    if (val.ToString() == "N")
                    {
                        bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_color_vis = 'P';
                    }
                    else if (val.ToString() == "R")
                    {
                        bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_color_vis = 'F';
                    }
                    else
                    {
                        bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault().taf_color_vis = null;
                    }
                }
            }
        }
        ...
        public bool ShowAudio
        {
            ...
            set
            {
                ...
                int size = (value ? 134 : 0) + 424 + (_ShowLab ? 99 : 0);
        ...
        public bool ShowLab
        {
            ...
            set
            {
                ...
                int size = (_ShowAudio ? 134 : 0) + 424 + (value ? 99 : 0);
                this.Size = new Size(911, size);
                tableLayoutPanel1.Size = new Size(911, size);
                panel4.AutoScrollMinSize = new Size(911, size);
                panel4.AutoScroll = true;
        ...
        private void load_aircrew_faa(ref trn_eye_aircrew_faa patient_aircrew_faa)
        {
            foreach (PropertyDescriptor prop in TypeDescriptor.GetProperties(patient_aircrew_faa))
            {
                eye_faa_PropertyChanged(patient_aircrew_faa, new PropertyChangedEventArgs(prop.Name));
            }

            patient_aircrew_faa.PropertyChanged += new PropertyChangedEventHandler(eye_faa_PropertyChanged);
        }
        private void eye_faa_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == "taf_color_vis")
            {
                var val = TypeDescriptor.GetProperties(sender)[e.PropertyName].GetValue(sender);
                if (val == null || val == DBNull.Value)
                {
                    TypeDescriptor.GetProperties(sender)["taf_fw_lantern"].SetValue(sender, null);
                    pnl_FW_FA.Enabled = false;
                }
                else
                {
                    if (val.ToString() == "F")
                    {
                        pnl_FW_FA.Enabled = true;
                    }
                    else
                    {
                        TypeDescriptor.GetProperties(sender)["taf_fw_lantern"].SetValue(sender, null);
        ...
        public void EndEdit()
        {
            ...
            trn_eye_aircrew_faa eyes_faa = bsEyeAircrewFAA.OfType<trn_eye_aircrew_faa>().FirstOrDefault();
            if (eyes_faa.taf_create_by == null)
            {
                eyes_faa.taf_create_by = user_name;
                eyes_faa.taf_create_date = dateNow;
            }
            eyes_faa.taf_update_by = user_name;
            eyes_faa.taf_update_date = dateNow;
        }
