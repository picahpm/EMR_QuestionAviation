...
{
    public partial class MammogramUC : UserControl
    {
        public MammogramUC()
        {
            InitializeComponent();
        ...
        public trn_patient_regi PatientRegis
        {
            ...
            set
            {
                ...
                else
                {
                    try
                    {
                        List<trn_mammogram> PatientBMD = value.trn_mammograms.ToList();

                        HisResult = PatientBMD
                                    .Where(x => x.tmg_en_no != value.tpr_en_no)
                                    .GroupBy(x => x.tmg_en_no)
                                    .Select((x, i) => new HistoryResult
                                    {
                                        en = x.Key,
                                        visitDate = x.Select(y => y.tmg_order_date.Value).OrderBy(y => y).FirstOrDefault()
                        ...
                        CurrentResultDetail curDtl = PatientBMD
                                                     ...
                                                     {
                                                         OrderDate = x.tmg_order_date,
                                                         OverseenBy = x.tmg_overseen_by,
                                                         ResultDate = x.tmg_result_date,
                                                         ResultName = x.tmg_order_name,
                                                         ResultText = x.tmg_result
                                                     }).FirstOrDefault();
