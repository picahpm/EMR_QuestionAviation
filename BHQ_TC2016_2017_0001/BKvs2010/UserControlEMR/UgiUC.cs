...
{
    public partial class UgiUC : UserControl
    {
        public UgiUC()
        {
            InitializeComponent();
        ...
        public trn_patient_regi PatientRegis
        {
            ...
            set
            {
                ...
                else
                {
                    try
                    {
                        List<trn_ugi_xray> PatientBMD = value.trn_ugi_xrays.ToList();

                        HisResult = PatientBMD
                                    .Where(x => x.tug_en_no != value.tpr_en_no)
                                    .GroupBy(x => x.tug_en_no)
                                    .Select((x, i) => new HistoryResult
                                    {
                                        en = x.Key,
                                        visitDate = x.Select(y => y.tug_order_date.Value).OrderBy(y => y).FirstOrDefault()
                        ...
                        CurrentResultDetail curDtl = PatientBMD
                                                     ...
                                                     {
                                                         OrderDate = x.tug_order_date,
                                                         OverseenBy = x.tug_overseen_by,
                                                         ResultDate = x.tug_result_date,
                                                         ResultName = x.tug_order_name,
                                                         ResultText = x.tug_result
                                                     }).FirstOrDefault();
