...
{
    public partial class EyesFrm : CheckupInheriteFrm
    {
        public EyesFrm()
        {
            InitializeComponent();
        ...
        private void EyesFrm_tpridChanged(object sender, int? e, CheckupInheriteFrm.tpr_idStatus? status)
        {
            if (e == null)
            {
                ...
                //tabEyesExamUC1.isDoctorRoom = true;
                tabEyesExamUC1.PatientRegis = null;                
                StatusEmptyRoom();
            }
            else
            {
                try
                {
                    ...
                    if (PatientRegis == null)
                    {
                        newPatientProfileStationUC1.tpr_id = null;
                        newPatientMappingUC1.tpr_id = null;
                        tabEyesExamUC1.isDoctorRoom = Program.CurrentRoom.mrd_type == 'D' ? true : false;
                        tabEyesExamUC1.PatientRegis = null;                        
                        StatusWK();
                    }
                    else
                ...
                {
                    Program.MessageError(this.Name, "EyesFrm_tpridChanged", ex, false);
                }
            }
        }
        private void EyesFrm_mrdidChanged(object sender, int? e)
        {
            if (this.FormStatus == formStatus.isStation)
            {
                if (e == null)
                ...
                else
                {
                    ...
                    {
                        Program.MessageError(this.Name, "EyesFrm_mrdidChanged", ex, false);
                    }
                }
            }
        }
                ...
                if (saveData())
                {
                    ...
                    else
                    {
                        if (new Class.FunctionDataCls().ChkSendAutoNewModule(Program.CurrentRegis))
                        {
                            string msgAlert = "";
                            bool isPopup = false;
                            List<int> mvt_id = new List<int> { (int)this.mvt_id };
                            ...
                            //{
                            //    WS_PathWay.SendAutoResult result = ws.SendAuto((int)tps_id, new List<int> { (int)mvt_id }.ToArray(), user.mut_username, Program.CurrentSite.mhs_id);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            string rptCodeEye = cls.getRptCodeEye((int)this.tpr_id);
            if (rptCodeEye != null) rptCode.Add(rptCodeEye);
            List<string> otherRptCode = cls.get_rptCodeEyeAviation((int)this.tpr_id);
